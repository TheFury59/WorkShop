...
if (isset($_GET['id'])) {
    ...
    $checkLike = $conn->prepare("SELECT * FROM likes WHERE idUser = ? AND idPost = ?");
    ...
    if ($result->num_rows == 0) {
        ...
        $addLike = $conn->prepare("INSERT INTO likes (idUser, idPost) VALUES (?, ?)");
        ...
        $removeLike = $conn->prepare("DELETE FROM likes WHERE idUser = ? AND idPost = ?");
